...
### 1. API Client Setup
- `[x]` Create `src/api/api.ts` with the base URL.
- `[x]` Implement an interceptor to retrieve the token via `supabase.auth.getSession()` and attach it to the `Authorization` header.
- `[x]` Retrieve `user_id` from local storage (or Auth context) to append to requests when needed.
...
- `[x]` **Login / Register** (`Auth` components): 
...
### 3. Stories Flow
- `[x]` **Get Library Stories** (`Library/index.tsx`): 
...
- `[x]` **Edit Story** (`Preview/index.tsx`): 
